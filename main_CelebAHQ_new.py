...
def get_args():
    ...
    parser.add_argument("--generate_images_H", help="whether to generate new latent codes", dest='actions', action='append_const', const='generate_images_H')
    ...
    default="/media/user/deeplearning/Advance-LSH/2026-dataset/celeba_HQ256/celeba_HQ_labels.csv")
    parser.add_argument("--data_root_train", help='path to training set directory (for AE)', type=str, metavar="", dest="data_root_train", 
    default="/media/user/deeplearning/Advance-LSH/2026-dataset/celeba_HQ256/trains/") #alltrain
    parser.add_argument("--data_root_test", help='path to testing set directory(for AE)', type=str, metavar="", dest= "data_root_test",
    default="/media/user/deeplearning/Advance-LSH/2026-dataset/celeba_HQ256/test/")  ## 
    ...
    parser.add_argument('--batch_size', type=int, default=64, help=' batch size of AE training')
    ...
    parser.add_argument('--attn_resolutions',  default=[16,]) ## [16,]
    ...
    parser.add_argument('--recnums', type=int, default=16,help='vistual numbers of reconstructed images')##
    ...
    parser.add_argument('--maxItersdot', type=int, default=5000,help='max iters of train ot')
    parser.add_argument('--lr_ot', type=int, default=50e-1,help='learning rate of calculate the update step based on gradient')
    #------ parameter of train OT-Net ------
    parser.add_argument('--maxIterotnet', type=int, default=1000,help='max iters of train ot')
    parser.add_argument('--lr_otnet', type=int, default=3e-3,help='learning rate of calculate the update step based on gradient')
    
    parser.add_argument('--bat_size_n', type=int, default=2000,help='Size of mini-batch of Monte-Carlo samples on device')
    parser.add_argument('--init_num_bat_n', type=int, default=20,help='Starting number of mini-batch of Monte-Carlo samples')
    parser.add_argument('--top_k', type=int, default=20, help='the nearest k samples around current sample')
    parser.add_argument('--angle_thresh', type=float, default=1.1,help='the threshold of the angle between two samples')
    parser.add_argument('--max_gen_samples', type=int, default=20000,help='max number of generated samples')#3000
    parser.add_argument('--dissim', type=float, default=0.2,help='the dissimilarity between the first generate new feature and the second generate new feature')#0.75
    parser.add_argument('--eps', type=int, default=4e-3,help='error of g_norm of train OT')
    parser.add_argument('--N', type=int, default=1,help='cyclic generation of random measures to obtain the height vector and measure')
    ...
    parser.add_argument('--netepochs', type=int, default=500, help='The whole Epochs for AE to train')#260
    parser.add_argument('--netbatch_size', type=int, default=256, help=' batch size of AE training')## 4096/2e-4/0.9
    parser.add_argument('--feature_dim', type=int, default=4096, help=' dimension of feature')
    parser.add_argument('--measure_dim', type=int, default=1, help=' dimension of measure embedding')
    parser.add_argument('--lr_net', type=float, default=4e-4, help='learning rate of AE training')##  
    parser.add_argument('--weight', default=0.0, type=float, help='weight decay of optimal Adam') 
    parser.add_argument('--train_ratio', type=float, default=0.93, help='training sample ratio') 
    #parser.add_argument('--result_root_path', type=str, default='./FFHQ1/', help='root path to save results')
    parser.add_argument('--hmodel', type=str, default='./CelebAHQ_new/h_models/Epoch_29_models.pth', help='root path to h models')
    ...
    parser.add_argument('--test_trainsample', default=False, type=str2bool, help='test h via pre-train model')
    parser.add_argument('--alter_measture', default=True, type=str2bool, help='achieving h via alter measure')
    parser.add_argument('--alter_measture_nums', default=29900, type=float, help='achieving h via alter measure')
    ...
    #------ generated images -------------
    parser.add_argument('--gen_nums', type=int, default=200,help='max number of generated samples')##
    parser.add_argument('--num_m', type=int, default=10,help='batch_size number of generated samples')##
    parser.add_argument('--gennums', type=int, default=25,help='vistual numbers of generated samples')##
    parser.add_argument('--Lambda', type=int, default=0.1, help='the nearest k samples around current sample')
    parser.add_argument('--label', type=int, default=1, help='Data categories contained in the raw data')
    
    #######################################FID and IS
    parser.add_argument('--result_root_path', type=str, default='./CelebAHQ_new/', help='root path to save results')
    parser.add_argument("--fid_cache", type=str, metavar="", dest= "fid_cache",default='./stats/Scelebahq.train.npz')
    parser.add_argument('--calculate_FID', default=True, type=str2bool, help='mu and sigma of calculated train data')
...
if __name__ == "__main__":
    ...
    if args.actions is None:
        actions = ['train_ae', 'extract_feature', 'train_SDOT', 'train_otnet', 'train_net_ot', 'generate_images','generate_images_H','generate_label_images']
    else:
    ...
    ot_height_path = os.path.join(args.result_root_path, 'h_ori.pt')   
    ...
    for action in actions:
        ...
        fileName = './CelebAHQ_new/CelebAHQ3.csv'
        ...
        if args.cons_samples:
            ...
            column_data = data['Glass']  ## Gender Glass   Expression
        ...
        if action == 'generate_images':
            #ot_height_path = os.path.join(args.result_root_path, 'h_pred.pt')
            generate_images(model,ae_model_path,ae_feature_path,ot_height_path,args)
            
        if action == 'generate_images_H':
            compute_sdot(ae_feature_path, alter_target_measure, idxs, args, mode='generate')
            decode_feature(model,ae_model_path,args)
        
        if action == 'generate_label_images':   
            generate_label_images(model,ae_model_path,ae_feature_path,ot_height_path,args)  
