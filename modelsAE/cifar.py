...
class autoencoder(nn.Module):
    ...
    def forward(self, inputs):
        inputs_noise = inputs #+ 0.02*torch.randn(inputs.size()).cuda() # add noise
        encoded = self.encoder(inputs_noise)
        #encoded = encoded + 0.01*torch.rand(encoded.size()).cuda() 
