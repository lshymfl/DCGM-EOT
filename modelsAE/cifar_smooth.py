...
class Encoder(nn.Module):  
    def __init__(self, dim_z=256, dim_c=3, dim_f=64, class_nums=10):
        super(Encoder, self).__init__()
        self.conv1 = nn.Sequential(nn.Conv2d(3, dim_f, 3, 2, 1),nn.LeakyReLU(0.2, inplace=True),)
        #self.conv1 = nn.Sequential(GaborLayer(3, dim_f, kernel_size=3, wavelength=3, orientation=45),nn.LeakyReLU(0.2, inplace=True),)  
        self.attention1 = NonLocalBlock(dim_f)  #ChannelAttention(dim_f)
        #self.rb1 = ResBlock(dim_f, 2*dim_f, 3, 2, 1, 'down') # 16,16 -->8,8
        self.rb1 = nn.Sequential(nn.Conv2d(dim_f, 2*dim_f, 3, 2, 1),nn.LeakyReLU(0.2, inplace=True),)
        self.attention2 = NonLocalBlock(2*dim_f)  #ChannelAttention(2*dim_f)
        #self.rb2 = ResBlock(2*dim_f, 4*dim_f, 3, 2, 1, 'down') # 8,8 -->4,4
        self.rb2 = nn.Sequential(nn.Conv2d(2*dim_f, 4*dim_f, 3, 2, 1),nn.LeakyReLU(0.2, inplace=True),) 
        self.attention3 = NonLocalBlock(4*dim_f)  #ChannelAttention(4*dim_f) 
        self.conv2 = nn.Conv2d(4*dim_f, dim_z, 4, 1, 0) # 4,4 -->100 1 1 
        #self.linear = nn.Linear(int(np.prod(image_shape)), dim_z) # 100 1 1                
        #self.dropout = nn.Dropout2d(p=0.2)

        self.classifier = nn.Sequential(nn.Linear(dim_z, 256), nn.PReLU(), nn.Linear(256, 128), nn.PReLU(), nn.Linear(128, class_nums), )    
        #self.classifier = nn.Sequential(nn.Linear(dim_z, 64), nn.PReLU(), nn.Linear(64, class_nums), )   ###  latent feature = 128
...
class Decoder(nn.Module):
    def __init__(self, dim_z=256, dim_c=3, dim_f=64):
        ...
        self.convT1 = nn.ConvTranspose2d(dim_z, 4*dim_f, 4, 1, 0) #  1,1 -->4,4 
        self.deatten1 = NonLocalBlock(4*dim_f)   #ChannelAttention(4*dim_f)
        #self.rb8 = ResBlock(4*dim_f, 2*dim_f, 2, 2, 0, 'up') # 4,4 -->8,8
        self.rb8 = nn.Sequential(nn.ConvTranspose2d(4*dim_f, 2*dim_f, 2, 2, 0),nn.ReLU(inplace=True),) 
        self.deatten2 = NonLocalBlock(2*dim_f)   #ChannelAttention(2*dim_f)
        #self.rb9 = ResBlock(2*dim_f, dim_f, 2, 2, 0, 'up') # 8,8 -->16,16 
        self.rb9 = nn.Sequential(nn.ConvTranspose2d(2*dim_f, dim_f, 2, 2, 0),nn.ReLU(inplace=True),)  
