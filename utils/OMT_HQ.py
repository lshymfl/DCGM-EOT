...
class OMT():	
    ...
    def __init__ (self, h_P, num_P, dim, max_iter, lr, bat_size_P, bat_size_n, eps, feature_indices):
    ...
    def cal_measure(self):
        ...
        while i < self.num_bat_P:  #self.num_P // self.bat_size_P:
            ...
            #temp_P = self.h_P[i*self.bat_size_P:(i+1)*self.bat_size_P,:,:,:]
            #self.d_temp_P.copy_(temp_P)
            #self.d_U = torch.einsum('ijkl,mjkl->im', self.d_temp_P, self.d_volP)
            ...
            '''add step'''
            i = i+1
            #print(self.d_tot_ind)
            #print(self.d_tot_ind.shape)
    ...
    def run_gd(self, last_step=0, num_bat=1):
        ...
        while(steps < self.max_iter):
            ...
            if g_norm < self.eps:
                return       
            ''' /h: save  intercept vector of brenier potential function
                /adam_m: save first moment 
                /adam_v: save second raw moment 
            '''
