...
class LSoftmaxLoss(nn.Module):
    def __init__(self, margin=4, num_classes=10):
        super(LSoftmaxLoss, self).__init__()
        self.margin = margin
        self.num_classes = num_classes
        self.m_lambda = 0.1  # scale factor 0.1

    def forward(self, logits, labels):
        batch_size = logits.size(0)
        index = torch.arange(0, batch_size, dtype=torch.long).to(labels.device)
        one_hot = torch.zeros_like(logits).scatter_(1, labels.view(-1, 1), 1)

        logits_m = logits.clone()
        logits_m[index, labels] -= self.margin * self.m_lambda

        loss = F.cross_entropy(logits_m, labels)
        return loss

    def update_lambda(self, epoch, max_epoch):
        self.m_lambda = min(1.0, float(epoch) / max_epoch)
...
class InfoNCELoss(nn.Module):
    ...
    def forward(self, z_i, z_j):
        ...
        sim = F.cosine_similarity(z.unsqueeze(1), z.unsqueeze(0), dim=2) / self.temperature  # (2N, 2N)

        # positive pairs: diagonal elements offset by N
        pos_sim = torch.cat([torch.diag(sim, N), torch.diag(sim, -N)]).view(2 * N, 1)

        # remove self-similarity
        mask = torch.eye(2 * N, dtype=torch.bool).to(z.device)
        all_sim = torch.exp(sim.masked_fill(mask, float('-inf')))  # (2N, 2N)

        # InfoNCE loss
        loss = -torch.log(torch.exp(pos_sim) / all_sim.sum(dim=1, keepdim=True)).mean()
        return loss
'''
